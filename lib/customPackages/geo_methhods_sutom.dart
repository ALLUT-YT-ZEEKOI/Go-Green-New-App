...
import 'package:gogreen_lite/customPackages/htto_clent.dart';
...
class GeoMethods {
  ...
  })  : assert(countryCode == '' || countryCode.length == 2, 'country must be passed as two character, ISO 3166-1 Alpha-2 compatible country code'),
        assert(countryCodes.length <= 5, 'just can filter up to 5 countries'),
        assert(countryCodes.every((c) => c.length == 2), 'countries must be passed as two character, ISO 3166-1 Alpha-2 compatible country code');
  ...
  Future<List<Address>> autocompletePlace({required String query}) async {
    ...
    if (query.isNotEmpty) {
      final queryParameters = {
        'key': googleApiKey,
        'language': language,
        'input': query + (city.isNotEmpty ? ', $city' : '') + (country.isNotEmpty ? ', $country' : ''),
      };
      if (countryCodes.isNotEmpty) {
        queryParameters['components'] = countryCodes.map((c) => 'country:$c').join('|');
      ...
      try {
        final response = await httpClient(uri);

        response['predictions'].forEach((element) => list.add(Address(
  ...
  Future<Address?> getPlaceGeometry({
    ...
    assert(placeId.isNotEmpty, "placeId can't be declared as an empty `String`");
    final uri = Uri.https(
      'maps.googleapis.com',
  ...
  Future<Directions?> getDirections({
    ...
    assert(countryCode.isNotEmpty, 'countryCode parameter is required to get Directions');
    assert(origin.hasCoords, 'origin has to contain coordinates values');
    assert(destination.hasCoords, 'destination has to contain coordinates values');
    String wps = '';
    if (waypoints.isNotEmpty) {
      waypoints.asMap().forEach((index, element) {
        if (element.hasCoords) {
          wps += (index != waypoints.length - 1) ? '${element.coords}|' : element.coords.toString();
        } else {
