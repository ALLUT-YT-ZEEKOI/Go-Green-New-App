...
class _SwipeableButtonViewState extends State<SwipeableButtonView> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    rippleController = AnimationController(vsync: this, duration: const Duration(milliseconds: 600));
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      width: isAccepted ? (MediaQuery.of(context).size.width - ((MediaQuery.of(context).size.width - 60) * _controller.value)) : double.infinity,
      height: 60,
      decoration: BoxDecoration(color: widget.isActive ? widget.activeColor : widget.disableColor, borderRadius: BorderRadius.circular(30)),
      child: Stack(
        children: [
          Align(
            alignment: Alignment.center,
            child: Opacity(opacity: opacity, child: Text(widget.buttonText, style: widget.buttontextstyle, textScaler: TextScaler.noScaling)),
          ),
          !isAccepted
              ? SwipeableWidget(
                  isActive: widget.isActive,
                  height: 60.0,
                  onSwipeValueCallback: (value) => setState(() => opacity = value),
                  child: SizedBox(
                    height: 60.0,
                    child: Padding(
                          ...
                          if (widget.isbutton)
                            Material(
                              ...
                              child: Container(
                                ...
                                child: widget.buttonWidgetend == null ? Center(child: opacity > 0.25 ? widget.buttonWidget : widget.buttonWidgetend) : Center(child: widget.buttonWidget),
                              ),
                            )
                          else
                            widget.buttonWidget
                  ...
                  onSwipeCallback: () {
                    ...
                    _controller.animateTo(1.0, duration: const Duration(milliseconds: 600), curve: Curves.fastOutSlowIn);
                  },
                )
              : AnimatedBuilder(
                  ...
                  builder: (context, child) => SizedBox(
                    ...
                    child: AnimatedBuilder(
                        ...
                        builder: (context, child) => Transform.scale(
                              ...
                              child: Container(
                                decoration: BoxDecoration(shape: BoxShape.circle, color: widget.activeColor.withOpacity(0.4)),
                                child: Padding(
                                  padding: const EdgeInsets.all(4.0),
                                  child: Container(
                                      decoration: BoxDecoration(shape: BoxShape.circle, color: widget.isActive ? widget.animationColor : widget.disableColor),
                                      child: Center(
                                        child: !isFinishValue ? CircularProgressIndicator(valueColor: widget.indicatorColor) : const SizedBox(),
                                      )),
                                ),
                              ),
                            )),
