...
class Lat {
  ...
  Map<String, dynamic> toJson() => {'latitude': latitude, 'longitude': logitude};

  factory Lat.fromJson(Map<String, dynamic> json) => Lat(latitude: json['latitude'], logitude: json['longitude']);
}
...
class MainProvider extends ChangeNotifier {
  ...
  void updatePickedsaved(UserAddress userAddress) {
    avilable = distanceInMeters(lat: userAddress.latitude, long: userAddress.longitude);
  ...
  Future<bool> updatePickedAddress(LatLong pickedData) async {
    ...
    avilable = distanceInMeters(lat: pickedData.latitude, long: pickedData.longitude);
    List<Placemark> placemarks = await placemarkFromCoordinates(pickedData.latitude, pickedData.longitude);
    if (placemarks.isNotEmpty) {
      pickedAddress = placemarks.first;
...
//make only one is true if click only once subscription o product page
  ...
  DateTime singleDate = DateTime.now().hour < 11 ? DateTime.now().add(const Duration(days: 1)) : DateTime.now().add(const Duration(days: 2));
  ...
  Future<void> loadAddresses() async {
    ...
    if (addressesJson != null) {
      ...
      List<Address> loadedAddresses = decodedAddresses.map((item) => Address(coords: item['coords'], bounds: item['bounds'], reference: item['reference'], placeId: item['placeId'])).toList();
  ...
  List<Map<String, dynamic>> addressesToMapList(List<Address> addresses) {
    return addresses.map((address) {
      return {'coords': address.coords?.toJson(), 'bounds': address.bounds?.toJson(), 'reference': address.reference, 'placeId': address.placeId};
    }).toList();
  }
  ...
  Future<void> saveAddresses(List<Address> newAddresses) async {
    ...
    List<Map<String, dynamic>> addressesMapList = addressesToMapList(combinedList);
